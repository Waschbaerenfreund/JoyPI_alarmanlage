...
MFRC522::MFRC522(){
  ...
  if (!mySPI->begin()){
    cout << "Error opening spi port 0!" << endl << flush;
    exit(-1);
  }
...
void MFRC522::setSPIConfig() {
  cout << "MFRC522::setSPIConfig" << endl << flush;
} // End setSPIConfig()
...
void MFRC522::PCD_WriteRegister(	uint8_t reg,		///< The register to write to. One of the PCD_Register enums.
  ...
  //cout << "PCD_WriteRegister(" << (int)reg << "," << (int)value << ") = " << (int)rx_buffer[0] << endl << flush;
...
uint8_t MFRC522::PCD_ReadRegister(	uint8_t reg	///< The register to read from. One of the PCD_Register enums.
  ...
  tx_buffer[0] = 
  tx_buffer[1] = (uint8_t) (0x80 | ((reg) & 0x7E));
  
  mySPI->xfer(tx_buffer,2,rx_buffer,2);
  return((uint8_t)rx_buffer[1]);
...
void MFRC522::PCD_ReadRegister(	uint8_t reg,		///< The register to read from. One of the PCD_Register enums.
  ...
  while (index < count) {
    ...
    mySPI->xfer(tx_buffer,1, rx_buffer,1);
  ...
  tx_buffer[0] = 0;
  mySPI->xfer(tx_buffer,1, rx_buffer,1);
  values[index] = rx_buffer[0];			// Read the final uint8_t. Send 0 to stop reading.
...
void MFRC522::PCD_Init() {
  if (digitalRead(NRSTPD) == LOW) {	//The MFRC522 chip is in power down mode.
    digitalWrite(NRSTPD, HIGH);		// Exit power down mode. This triggers a hard reset.
